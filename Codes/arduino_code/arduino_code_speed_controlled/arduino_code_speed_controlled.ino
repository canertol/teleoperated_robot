  ...
  while (Serial.available() > 0)
  {
   ...
   // Serial.println(data);
    ...
    if(ind == 0)
       ...
       if (abs(mag*cos(theta))<50)
        stick_data = 0;
       else if(mag * cos(theta) > 87){
        stick_data = 10;
        }
       else if(mag * cos(theta) > 50 && mag * cos(theta) < 87){
        stick_data = 20;
       } 
       else if(mag * cos(theta) < -85){
        stick_data = 5;
        }
       else if(mag * cos(theta) < -50 && mag * cos(theta) > -85){
        stick_data = 15;
        } 
       else if(mag * sin(theta) > 85){
        stick_data = 0;
       }
       else if(mag * sin(theta) < -80){
        stick_data = 0;      
       }
    }
...
int sensors(int trigPin1,int trigPin2, int trigPin3, int trigPin4,int echoPin1, int echoPin2,int echoPin3,int echoPin4 )
{
  ...
  if(distance1 < threshold && MSG != prevMSG){
    MSG = 1;
    Serial.write("1");
    prevMSG = MSG;}
  if(distance2 < threshold && MSG != prevMSG){
    MSG = 2;
    Serial.write("2");
    prevMSG = MSG;}
  if(distance3 < threshold && MSG != prevMSG){
    MSG = 3;
    Serial.write("3");
    prevMSG = MSG;}
  if(distance4 < threshold && MSG != prevMSG){
    MSG = 4;
    Serial.write("4");
    prevMSG = MSG;}
 /* if( distance1 > threshold && distance2 > threshold && distance3 > threshold && distance4 > threshold && MSG != prevMSG){
    MSG = 0;
    Serial.write("0");
    prevMSG = MSG;
  }*/
